# forecast_13w_multi_models_backtest_with_plots_pi.py
...
# =========================
# CONFIG (WEEKLY, 13W FORWARD)
# =========================
HISTORY_FILE      = "History_Weekly.csv"   # ProductID, ChannelID, LocationID, StartDate, EndDate, Qty

# Outputs (CSV)
OUT_BACKTEST_CSV  = "Backtest_Weekly_Predictions_ByModel.csv"
OUT_SUMMARY_CSV   = "Model_Selection_Summary_Weekly_ByCombo.csv"
OUT_FORWARD_CSV   = "Future13W_Forecasts_BestModel.csv"

# Plots
PLOTS_DIR_BACKTEST = os.path.join("plots", "backtest_weekly")
PLOTS_DIR_FORWARD  = os.path.join("plots", "history_plus_forecast_weekly")

# Feature engineering
LAG_WINDOW        = 4            # last 4 weeks
USE_SEASONAL_L12  = True         # interpreted as "use seasonal lag" (we’ll use SEASONAL_LAG)
ADD_MONTH_DUMMIES = False        # optional for weekly; you could swap to week-of-year dummies if needed

# Seasonal config
SEASONAL_LAG      = 52           # weekly seasonal lag (~annual)
SEASONAL_PERIOD   = 52           # period for seasonal_naive

# Backtest & selection
MIN_TRAIN_POINTS  = 26           # at least ~half a year
CV_SPLITS_MIN     = 3
METRIC            = "WMAPE"
FAST_MODE         = True
CV_STRIDE         = 2
...
# Forward horizon
FORWARD_HORIZON   = 13           # next 13 weeks
...
def make_design_matrix(dates: pd.DatetimeIndex, y_log: pd.Series,
    ...
    # "Seasonal" lag based on SEASONAL_LAG (here 52 weeks)
...
# Seasonal naive with configurable period (here 52 weeks)
...
def forward_forecast_best(series: pd.Series, best_model_name: str, horizon: int = FORWARD_HORIZON):
    ...
    for _ in range(horizon):
        # Weekly: next completed week
        next_dt = hist_lin.index.max() + relativedelta(weeks=1)
...
def plot_backtest_combo(pid, chanid, locid, series, preds_dict, out_dir=PLOTS_DIR_BACKTEST):
    ...
    plt.title(f"Backtest (Aligned, Weekly) | {pid} / {chanid} / {locid}")
...
def plot_history_plus_forecast(pid, chanid, locid, hist_df, fut_df, out_dir=PLOTS_DIR_FORWARD,
    ...
    if len(sub_fut):
        ...
        plt.plot(fut_dates, fut_vals, label=f"Forecast 13W ({best_model})", linewidth=2)
    ...
    plt.title(f"History + 13W Forecast | {pid} / {chanid} / {locid}")
    plt.xlabel("Date"); plt.ylabel("Qty")
    plt.grid(True, alpha=0.3)
    plt.legend(loc="best")
    plt.tight_layout()
...
def run():
    ...
    for _, rr in combos.iterrows():
        ...
        # ---- Forward 13W using best model (weekly)
        combo_tab = hist[(hist["ProductID"]==pid) & (hist["LocationID"]==locid) & (hist["ChannelID"]==chanid)]
        last_start = combo_tab["StartDate"].max()

        future_starts = [last_start + relativedelta(weeks=i+1) for i in range(FORWARD_HORIZON)]
        future_ends   = [(dt + relativedelta(weeks=1)) - pd.Timedelta(days=1) for dt in future_starts]
    ...
    if backtest_rows:
        ...
        print(f"[OK] Backtest (weekly) written: {OUT_BACKTEST_CSV} | rows={len(df_bt)}")
    else:
        print("[WARN] No backtest rows written (weekly).")

    if summary_rows:
        df_sm = pd.DataFrame(summary_rows)
        df_sm.to_csv(OUT_SUMMARY_CSV, index=False)
        print(f"[OK] Summary (weekly) written: {OUT_SUMMARY_CSV} | rows={len(df_sm)}")
    else:
        print("[WARN] No summary rows written (weekly).")
    ...
    if forward_rows:
        ...
        print(f"[OK] Forward 13W written: {OUT_FORWARD_CSV} | rows={len(df_fw)}")
    else:
        print("[WARN] No forward rows written (weekly).")

    print(f"[OK] Weekly plots saved to:\n  - {PLOTS_DIR_BACKTEST}\n  - {PLOTS_DIR_FORWARD}")
