# forecast_18m_multi_models_backtest_with_plots_pi.py
...
# CONFIG
# =========================
HISTORY_FILE      = "History_Monthly.csv"   # ProductID, ChannelID, LocationID, StartDate, EndDate, Qty

# Outputs (CSV)
OUT_BACKTEST_CSV  = "Backtest_Predictions_ByModel.csv"
OUT_SUMMARY_CSV   = "Model_Selection_Summary_ByCombo.csv"
OUT_FORWARD_CSV   = "Future18M_Forecasts_BestModel.csv"

# Plots
PLOTS_DIR_BACKTEST = os.path.join("plots", "backtest")
PLOTS_DIR_FORWARD  = os.path.join("plots", "history_plus_forecast")

# Feature engineering
LAG_WINDOW        = 3
USE_SEASONAL_L12  = True
...
# HELPERS
# =========================
def parse_dt_exact(s): return pd.to_datetime(s, dayfirst=True, errors="coerce")
...
# Seasonal naive (monthly)
def seasonal_naive(series: pd.Series, horizon: int):
    vals = series.values.astype(float); out = []
    for h in range(1, horizon+1):
        if len(vals) >= 12:
            idx = len(vals) - 12 + (h-1)
            out.append(vals[idx] if idx < len(vals) else (out[-12] if len(out)>=12 else vals[-1]))
        else:
...
def next_feature_row_linear_space(hist_lin: pd.Series, next_dt: pd.Timestamp,
    ...
    if use_l12:
        val12 = hist_lin.iloc[-12] if len(hist_lin) >= 12 else np.nan
        feats["lag12"] = np.log1p(max(0.0, float(val12))) if pd.notna(val12) else 0.0
    if add_months:
        m = int(next_dt.month)
        for j in range(1, 13):
...
def forward_forecast_best(series: pd.Series, best_model_name: str, horizon: int = FORWARD_HORIZON):
    ...
    y_log = np.log1p(np.maximum(0.0, series.values))
    X_all, y_all, idx_all, cols = make_design_matrix(series.index, pd.Series(y_log, index=series.index),
                                                     LAG_WINDOW, USE_SEASONAL_L12, ADD_MONTH_DUMMIES)
    ...
    for _ in range(horizon):
        next_dt = hist_lin.index.max() + relativedelta(months=1)
...
def plot_backtest_combo(pid, chanid, locid, series, preds_dict, out_dir=PLOTS_DIR_BACKTEST):
    ...
    plt.title(f"Backtest (Aligned) | {pid} / {chanid} / {locid}")
...
def plot_history_plus_forecast(pid, chanid, locid, hist_df, fut_df, out_dir=PLOTS_DIR_FORWARD,
    ...
    if len(sub_fut):
        ...
        plt.plot(fut_dates, fut_vals, label=f"Forecast 18M ({best_model})", linewidth=2)
    ...
    plt.title(f"History + Forecast | {pid} / {chanid} / {locid}")
    plt.xlabel("Date"); plt.ylabel("Qty")
    plt.grid(True, alpha=0.3)
    plt.legend(loc="best")
    plt.tight_layout()
...
def run():
    ...
    for _, rr in combos.iterrows():
        ...
        # ---- Forward 18M using best model
        combo_tab = hist[(hist["ProductID"]==pid) & (hist["LocationID"]==locid) & (hist["ChannelID"]==chanid)]
        last_start = combo_tab["StartDate"].max()
        future_starts = [last_start + relativedelta(months=i+1) for i in range(FORWARD_HORIZON)]
        future_ends   = [(dt + relativedelta(months=1)) - pd.Timedelta(days=1) for dt in future_starts]
    ...
    if backtest_rows:
        ...
        print(f"[OK] Backtest written: {OUT_BACKTEST_CSV} | rows={len(df_bt)}")
    else:
        print("[WARN] No backtest rows written.")

    if summary_rows:
        df_sm = pd.DataFrame(summary_rows)
        df_sm.to_csv(OUT_SUMMARY_CSV, index=False)
        print(f"[OK] Summary written: {OUT_SUMMARY_CSV} | rows={len(df_sm)}")
    else:
        print("[WARN] No summary rows written.")
    ...
    if forward_rows:
        ...
        print(f"[OK] Forward 18M written: {OUT_FORWARD_CSV} | rows={len(df_fw)}")
    else:
        print("[WARN] No forward rows written.")

    print(f"[OK] Plots saved to:\n  - {PLOTS_DIR_BACKTEST}\n  - {PLOTS_DIR_FORWARD}")
